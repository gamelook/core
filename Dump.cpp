...
//TCHAR g_szVersion[] = _T("1.0.20070518");
//

LONG WINAPI TopLevelFilter(_EXCEPTION_POINTERS *pExceptionInfo, const char *file)
{
	//time_t now;
	//time(&now);
	//tm tmNow;
	...
	//_stprintf_s(szDumpFile, 
	//	_T("%s-%s-%d-%s%d-%s%d-%s%d-%s%d-%s%d.dmp"),
	//	g_szDumpFile, 
	//	g_szVersion, 
	//	tmNow.tm_year + 1900, 
	//	tmNow.tm_mon + 1 > 9 ? "" : "0",
	...
	HANDLE hFile = CreateFileA(file, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, 
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		MiniDumpWriteDump(
			GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, &exceptionInfo,
			NULL, NULL);
...
#include <stdlib.h>
 ...
 #define CRASHREPORT_EMAIL "89018@qq.com"
 
 #define DBGHELP_DLL "dbghelp.dll"
 
 #define VERSION_DLL "version.dll"
 
 static char LOGFILE_PREFIX[1024] ="crash@";
 ...
 convert_wbcs_to_utf8(const wchar_t *str)
 {
 	...
 	wcstombs_s(&len, utf8_str, sizeof(wchar_t) * len, str, len);
 			   ...
 			   PMINIDUMP_CALLBACK_OUTPUT callback_output)
 {
 		...
 		)
 	{
 		...
 		WideCharToMultiByte( CP_ACP, 0, module.FullPath, -1, buf, 1023 ,NULL, NULL);
...
#define INNER_SWITCH do \
{\
	switch(basic_type)\
	{\
	case 2:  /* btChar */\
		{\
			if (!IsBadStringPtrA(*(PSTR*)address, 32))\
...
{\
	DWORD bt;\
	ULONG64 length;\
	SymGetTypeInfo(proc, mod_base, type, TI_GET_LENGTH, &length);\
\
	/* print a char * as a string */\
	if (ptr == 1 && length == 1)\
	{\
		sprintf(value_str, "0x%08x \"%s\"",\
			*(DWORD *)value_addr, (char*)value_addr);\
		break;\
	}\
	if (ptr >= 1)\
	{\
	...
	if (SymGetTypeInfo(proc, mod_base, type, TI_GET_BASETYPE, &bt))\
	{\
...
{\
	...
	if (SymGetTypeInfo(proc, mod_base, type, TI_GET_SYMNAME,\
		&type_name_wbcs))\
	{\
		char type_name[1024];type_name[1023]=0;\
		WideCharToMultiByte( CP_ACP, 0, type_name_wbcs, -1, type_name, 1023 ,NULL, NULL);\
		LocalFree(type_name_wbcs);\
\
		if (ptr == 0)\
			sprintf(value_str, "(%s) 0x%08x",\
			type_name, PtrToInt(value_addr));\
		else if (ptr == 1)\
			sprintf(value_str, "(%s *) 0x%08x",\
 ...
 format_value(char *value_str, DWORD64 mod_base, DWORD type, void *value_addr)
 {
 	...
 	while (SymGetTypeInfo(proc, mod_base, type, TI_GET_SYMTAG, &tag))
 	{
 		/* SymTagPointerType */
 		if (tag == 14)
 		{
 			ptr++;
 			SymGetTypeInfo(proc, mod_base, type, TI_GET_TYPE, &type);
 			continue;
 		}
 		break;
 	}
 ...
 write_function_detail(STACKFRAME stack_frame, void *data)
 {
 	...
 	if (SymFromAddr(proc, stack_frame.AddrPC.Offset, &func_disp, pIHS) == TRUE)
 	{
 		fprintf(log_file,
 			"#%d  0x%08x in %.200s (",
 		...
 		SymSetContext(proc, &ih_stack_frame, 0);
 		...
 		SymEnumSymbols(proc, 0, 0, write_var_values, &ensym);
 	...
 	if (SymGetLineFromAddr(proc, stack_frame.AddrPC.Offset,
 		&line_disp, &ih_line) != 0)
 	{
 		fprintf(log_file,
...
#else
 #error This crash handler only works on Win32 and Win64.
#endif
 
 	while (1)
 	{
 		if (!StackWalk(machine, proc, GetCurrentThread(),
 				&stack_frame, context, NULL, SymFunctionTableAccess,
 ...
 load_dbghelp_dll()
 {
	...
	/* Set the 'load lines' option to retrieve line number information;
	   ...
	   when needed. */
	SymSetOptions(opts | SYMOPT_LOAD_LINES | SYMOPT_DEFERRED_LOADS);
 ...
 get_temp_filename(char *filename, const char *prefix, const char *ext)
 {
 	...
 	for (i = 0;i < 3;i++)
 	{
 		...
 		file = CreateFileA(filename, GENERIC_WRITE, 0, NULL, CREATE_NEW,
 ...
 LONG WINAPI
 unhandled_exception_filter(PEXCEPTION_POINTERS ptrs)
 {
	...
	TopLevelFilter(ptrs, dmp_filename);
 ...
 void init_exception_by_pdb(void)
 {
...
namespace core
{
	void DbgDumpInstall(const char* szDumpFile)
	{
#ifdef WIN32
		strncpy(LOGFILE_PREFIX, szDumpFile,sizeof(LOGFILE_PREFIX) );
		LOGFILE_PREFIX[sizeof(LOGFILE_PREFIX)-1] = 0;
		SetUnhandledExceptionFilter(unhandled_exception_filter);
#endif
